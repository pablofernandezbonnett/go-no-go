...
Local CLI invocation forms:

- `./gradlew run --args="<subcommand and flags>"`
- `./build/install/go-no-go-engine/bin/go-no-go-engine <subcommand and flags>` after `./gradlew installDist`

The logical root command name is `gonogo`, but the local installed launcher generated by this repository is `go-no-go-engine`.
