...
Salary opacity remains a strong negative signal by default and can still be configured as a persona hard filter when required.

## Current Stack

- Java 21 (default, configurable)
- Gradle (wrapper)
- Picocli (CLI)
- JLine (terminal UI)
...
- CLI entrypoint (Picocli root command exposed locally via Gradle or the installed `go-no-go-engine` launcher)
...
- `fetch` subcommand to normalize raw text into YAML
- `fetch-web` subcommand to fetch selected career pages from `config/companies.yaml`
- `evaluate` subcommand (persona-aware)
- `evaluate-input` subcommand for direct URL/raw-text evaluation
- `rerun-ad-hoc` subcommand to refresh saved `output/ad-hoc-evaluations/*.yaml` artifacts
- `check` short command with input autodetection (URL / raw text / file path)
- `evaluate-batch` subcommand with markdown/json report generation
- `weekly-digest` subcommand from batch JSON reports
- `pipeline run` end-to-end orchestration
- `pipeline run-all` one-shot orchestration for all personas + all companies
...
- `schedule` subcommand to generate non-active scheduled-run artifacts (script + cron file)
- `tui` interactive terminal launcher for common commands with config-backed defaults
...
- Advanced command guide: [`../../docs/advanced-guide.md`](../../docs/advanced-guide.md)
...
- The logical root command name is `gonogo`.
- The local installed launcher generated by this repository is `go-no-go-engine`.
- The advanced guide includes `check`, `pipeline run`, `rerun-ad-hoc`, UI launch helpers, and port/env-var notes.
...
Equivalent installed-launcher command:

```bash
./build/install/go-no-go-engine/bin/go-no-go-engine rerun-ad-hoc --input-dir output/ad-hoc-evaluations
```
...
- The default Java toolchain version is pinned in `gradle.properties` as `21`.
- You can override the Java toolchain version with `-PgonogoJavaVersion=<major>` (example: `-PgonogoJavaVersion=25`).
