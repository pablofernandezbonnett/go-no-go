...
- `Evaluate`: submit a URL or pasted job text, compare results across one persona or all personas, and reuse recent URLs. Saved evaluations lead with a plain-English summary instead of raw engine internals.
- `Batch`: scan verdicts and scores across one report run.
- `Job Detail`: inspect one evaluated job through a plain-English summary, visible factors, verdict, and score.
